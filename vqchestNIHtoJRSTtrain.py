...
from convnets import GumbelUNet2Dpos,  VQUNet2Dpos
...
class JSRTDataloader(LightningDataModule):
    def __init__(self, batch_size, csv_names,base_paths, csv_paths, num_workers=8, **kwargs):
        ...
        self.datasets1 = NIHDataset(csv_name=csv_names[2],base_path = base_paths[1], csv_path = csv_paths[1],  **kwargs) 
        self.datasets2 = JSRTDataset(csv_name=csv_names[0],base_path = base_paths[0], csv_path = csv_paths[0],  **kwargs)
        self.datasets3 = JSRTDataset(csv_name=csv_names[1],base_path = base_paths[0], csv_path = csv_paths[0],  **kwargs)
...
#        self.automatic_optimization = False
    ...
    def test_step(self, batch, batch_idx):
       ...
       # dice1, num_items1, hd1, asd1 = 0, 0, 0, 0
        ...
        imagespath = '/vol/biomedic3/as217/vqseg/outputchestsegfinal/vqNIHtoJRST/testimagesoutput/'
        ...
        os.makedirs('/vol/biomedic3/as217/vqseg/outputchestsegfinal/vqNIHtoJRST/testresults/', exist_ok=True)
        df.to_csv( '/vol/biomedic3/as217/vqseg/outputchestsegfinal/vqNIHtoJRST/testresults/result.csv')
        for i in range(len(outputs)):
           torchvision.utils.save_image(outputs[i], os.path.join(imagespath, 'out{img}batch{batch}.png'.format(img = i, batch = batch_idx)))
           torchvision.utils.save_image(torch.squeeze(images[i,:,:,:]), os.path.join(imagespath, 'image{img}batch{batch}.png'.format(img = i, batch = batch_idx)))
           torchvision.utils.save_image(torch.squeeze(labels[i,:,:,:]), os.path.join(imagespath, 'labels{img}batch{batch}.png'.format(img = i, batch = batch_idx)))
...
if    __name__ == '__main__':
    ...
    csv1 = ["JSRT_{}.csv".format(_set) for _set in ['val','test']]
    csv2 = ["{}_NIH.csv".format(_set) for _set in [ 'train']]
    net = Net()
    data = JSRTDataloader(
        batch_size=10,
        num_workers=4,
    ...
    root_dir = 'outputchestsegfinal/vqNIHtoJRST'
        # set up checkpoints
    ...
    os.makedirs('/vol/biomedic3/as217/vqseg/outputchestsegfinal/vqNIHtoJRST/testimagesoutput/ ', exist_ok=True)
    net = Net()
