...
class ProstateDataset(Dataset):
    ...
    def get_sample(self, item):
        ...
        """
        image = nib.load(sample['image'])
        i = image.affine
        image = np.array(image.get_fdata()).astype(np.float32)
        label = nib.load(sample['label'])
        l = label.affine
        label = np.array(label.get_fdata()).astype(np.float32)
        sample = {'image': np.expand_dims(image, axis=0), 'label': np.expand_dims(label, axis=0)}
        """
...
#        self.automatic_optimization = False
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {"val_loss": loss, "val_dice": self.dice_metric.aggregate().item(), "hd": hd, "asd": asd, "val_number": len(outputs)}
    ...
    def test_step(self, batch, batch_idx):
        ...
        imagespath = '/vol/biomedic3/as217/vqseg/outputprostatefinal/gumbelvqBCMtoRUM/testimagesoutput/'
        ...
        os.makedirs('/vol/biomedic3/as217/vqseg/outputprostatefinal/gumbelvqBCMtoRUM/testresults/', exist_ok=True)
        df.to_csv('/vol/biomedic3/as217/vqseg/outputprostatefinal/gumbelvqBCMtoRUM/testresults/result.csv')
...
if __name__ == '__main__':
        ...
        data = ProstateDataModule(
            ...
            csv_val_img="/vol/biomedic3/as217/vqseg/ProstateDomain/ProstatedomainvalRUNMC1.csv",
            csv_test_img="/vol/biomedic3/as217/vqseg/ProstateDomain/Prostatedomaints1.csv",
        )

        root_dir = 'outputprostatefinal/gumbelvqBCMtoRUM'
        # set up checkpoints
        ...
        os.makedirs('/vol/biomedic3/as217/vqseg/outputprostatefinal/gumbelvqBCMtoRUM/testimagesoutput/ ', exist_ok=True)
        net = Net()
